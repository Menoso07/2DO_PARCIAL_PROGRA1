...
def crear_diccionario_alumno(linea:str,separador:str =",") -> dict:
    ...
    alumno = {}
    alumno["pregunta"] = lista_datos[0]
    alumno["respuesta_1"] = lista_datos[1]
    alumno["respuesta_2"] = lista_datos[2]
    alumno["respuesta_3"] = lista_datos[3]
    alumno["respuesta_4"] = lista_datos[4]
    alumno["respuesta_correcta"] = int(lista_datos[5])
...
def obtener_top_10(path_json:str) -> list:
    ...
    Incluye la cabecera como primer fila.
    """
    try:
        ...
        cabecera = matriz[0]
        datos = matriz[1:]
        # Ordenar por score (índice 1) de mayor a menor
        datos_ordenados = sorted(datos, key=lambda x: int(x[1]), reverse=True)
        top_10 = [cabecera] + datos_ordenados[:10]
        return top_10
...
def crear_lista_powerups(textura:str, ancho:int, alto:int, x:int, y:int, cantidad:int=4):
    lista = []
    for i in range(cantidad):
        elemento = crear_elemento_juego(textura, ancho, alto, x, y + i*(alto+10))
        lista.append(elemento)
    return lista

def obtener_powerups():
    powerups = []
    with open("powerups.csv", newline='', encoding='utf-8') as csvfile:
        reader = csv.reader(csvfile)
        next(reader)  # Salta encabezado
        for fila in reader:
            # Suponiendo que los valores están en columnas 1 a 4
            powerups.append(fila[1:5])
    return powerups
